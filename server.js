...
app.get('/api/admin/users', ...adm, async (req, res) => {
  const users = await all('SELECT id,username,email,role,is_banned,xp,coins,levels_completed,rank_points,wins,losses,created_at FROM users ORDER BY created_at DESC');
  res.json(users.map(u => ({ ...u, is_online: onlineUsers.has(u.id) })));
});
...
io.on('connection', async socket => {
  ...
  socket.on('disconnect', () => { waiting.delete(uid); onlineUsers.delete(uid); });
});
