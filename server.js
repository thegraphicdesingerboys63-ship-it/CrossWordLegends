...
async function initDb() {
  const stmts = [
    ...
    `ALTER TABLE users ADD COLUMN IF NOT EXISTS hide_online INTEGER DEFAULT 0`,
...
app.get('/api/game/me', auth, async (req, res) => {
  const user = await get('SELECT id,username,email,role,is_banned,xp,coins,levels_completed,rank_points,wins,losses,active_title,active_icon,active_card_bg,bio,daily_streak,hide_online,created_at FROM users WHERE id=?', [req.user.id]);
...
// ── USER SETTINGS ─────────────────────────────────────────────────────────────
app.patch('/api/settings/username', auth, async (req, res) => {
  try {
    ...
    const existing = await get('SELECT id FROM users WHERE username=? AND id!=?', [username, req.user.id]);
    if (existing) return res.status(409).json({ error: 'Username already taken' });
    await run('UPDATE users SET username=? WHERE id=?', [username, req.user.id]);
    const user = await get('SELECT id,username,email,role,is_banned,xp,coins,levels_completed,rank_points,wins,losses,active_title,active_icon,active_card_bg,bio,daily_streak,created_at,hide_online FROM users WHERE id=?', [req.user.id]);
    res.json({ success: true, user: sanitize(user) });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.patch('/api/settings/email', auth, async (req, res) => {
  try {
...
app.patch('/api/settings/privacy', auth, async (req, res) => {
  try {
    const { hideOnline } = req.body;
    await run('UPDATE users SET hide_online=? WHERE id=?', [hideOnline ? 1 : 0, req.user.id]);
    res.json({ success: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/friends', auth, async (req, res) => {
  const uid = req.user.id;
  const rows = await all(`SELECT u.id,u.username,u.xp,u.rank_points,u.levels_completed,u.hide_online,f.status,CASE WHEN f.user_id=? THEN 'sent' ELSE 'received' END as direction FROM friendships f JOIN users u ON (u.id=CASE WHEN f.user_id=? THEN f.friend_id ELSE f.user_id END) WHERE f.user_id=? OR f.friend_id=?`, [uid, uid, uid, uid]);
  res.json(rows.map(r => ({ ...r, is_online: !r.hide_online && onlineUsers.has(r.id) })));
});
...
// ── DAILY CHALLENGE ───────────────────────────────────────────────────────────
const DAILY_PERIOD = 24 * 3600 * 1000; // 24 hours in ms

app.get('/api/daily/status', auth, async (req, res) => {
  try {
    const last = await get('SELECT completed_at FROM daily_challenge_completions WHERE user_id=? ORDER BY completed_at DESC LIMIT 1', [req.user.id]);
    const user = await get('SELECT daily_streak FROM users WHERE id=?', [req.user.id]);
    const now = Date.now();
    const lastMs = last ? new Date(last.completed_at).getTime() : 0;
    const elapsed = now - lastMs;
    const completed = last && elapsed < DAILY_PERIOD;
    const nextResetMs = completed ? DAILY_PERIOD - elapsed : 0;
    res.json({ completed: !!completed, streak: user?.daily_streak || 0, nextResetMs });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/daily/complete', auth, async (req, res) => {
  try {
    const uid = req.user.id;
    const now = Date.now();
    const last = await get('SELECT completed_at FROM daily_challenge_completions WHERE user_id=? ORDER BY completed_at DESC LIMIT 1', [uid]);
    const lastMs = last ? new Date(last.completed_at).getTime() : 0;
    if (last && (now - lastMs) < DAILY_PERIOD) return res.status(409).json({ error: 'Already completed in the last 24 hours' });
    const { score = 0, wordsFound = 0 } = req.body;
    // Streak: last completion between 24h and 48h ago = continues; >48h ago or none = reset
    const user = await get('SELECT daily_streak FROM users WHERE id=?', [uid]);
    const hoursSinceLast = (now - lastMs) / 3600000;
    const newStreak = (!last || hoursSinceLast > 48) ? 1 : (user.daily_streak || 0) + 1;
    ...
    await run('INSERT INTO daily_challenge_completions (user_id,date_str,score,words_found) VALUES (?,?,?,?)', [uid, dateStr, score, wordsFound]);
    await run('UPDATE users SET daily_streak=?,last_daily_date=?,coins=coins+?,xp=xp+?,levels_completed=levels_completed+1 WHERE id=?', [newStreak, dateStr, totalCoins, baseXp, uid]);
    // Streak milestones
